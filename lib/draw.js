...
export function drawCard(ctx, opts) {
  ...
  const mascot = opts.mascot || null;   // 브랜드 마스코트 (우산 쓴 고양이)
  ...
  const info = { dreamLines: [], dreamSize: 0, dreamFits: true, photoRect: null, captionH: 0, photoArea: null, mascot: false };
  ...
  // 마스코트 — 브랜드 이미지가 준비되면 그것을, 아직이면 선 그림을 그립니다.
  let textX = 430;
  if (mascot && mascot.complete && mascot.naturalWidth) {
    const mh = 376;
    const mw = (mascot.naturalWidth / mascot.naturalHeight) * mh;
    const mx = 78;
    const my = cyc - mh / 2 - 26;
    ctx.save();
    ctx.shadowColor = "rgba(57,48,76,.20)";
    ctx.shadowBlur = 26;
    ctx.shadowOffsetY = 10;
    ctx.drawImage(mascot, mx, my, mw, mh);
    ctx.restore();
    info.mascot = true;
    textX = Math.round(mx + mw + 24);
  } else {
  ...
  ctx.shadowBlur = 22;
  ctx.fillText("화이팅!", textX, cyc + 38);
  ctx.restore();
  ctx.fillStyle = "#6B4AA8";
  ctx.font = '400 32px "Gowun Dodum", sans-serif';
  ctx.fillText("당신의 꿈을 크게 응원합니다", textX + 6, cyc + 96);
  ctx.textAlign = "center";
